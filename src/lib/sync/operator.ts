...
export const handleSync = async function (
  ...
  try {
    ...
    if (isBackgroundActivityClosedError(error)) {
      ...
      dump("[RoundExit] abandoned mid-round: backgroundActivityClosed (silent-exit fingerprint, was at phase=" + plugin.syncState.syncPhase + ", needsFullRevoked=" + String(plugin.incrementalScanManager?.canUseIncrementalSync(true) === false) + ")");
      return;
    }
